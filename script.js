...
function criarGraficos(feeds){
    ...
    const horas = feeds.map(f =>
        new Date(f.created_at).getHours() + "h"
    );
...
function carregarDica(){
    ...
    .then(lista => {
        ...
        if(localStorage.getItem("dataDica") !== hoje){
            ...
            let filtradas =
                lista.filter(d => d.condicao === estadoAtual);
        ...
        const dica =
            JSON.parse(localStorage.getItem("dicaHoje"));

        document.getElementById("dicaTitulo").innerText =
            dica.titulo;
...
legenda.onAdd = function () {
    ...
    div.innerHTML += "Chuva<br>";
    div.innerHTML += "Fraca<br>";
    div.innerHTML += "Moderada<br>";
    div.innerHTML += "Forte<br>";
    div.innerHTML += "Muito forte<br>";
